...
def parse_archive_order(order_json: json) -> Order:
    try:
        # turning meta_data from array of single dictionaries to a large dictionary to index
        ...
        # pdata will contain client information (part of response dictionary)
        pdata = order_json['billing']

        # getting middle initial here since it's going to be empty most of the time anyway
        name = Name(xstr(pdata['first_name']),
                    xstr(pdata['last_name']),
                    '')
        try:
            dob = parser.parse(wc_metadata['patients_date_of_birth'])
        except Exception as e:
            raise BadPHIException(e)

        address = Address(street_1=(pdata['address_1']),
                          street_2=(pdata['address_2']),
                          city=(pdata['city']),
                          state=xstr(pdata['state']).upper(),
                          zip_code=xstr(pdata['postcode']),
                          country=(pdata['country']))

        sex_dict = {'MALE': Sex.MALE, 'FEMALE': Sex.FEMALE}
        ethnicity_dict = {'HISPANIC/LATINO': Ethnicity.HISPANIC_LATINO,
                          'NON-HISPANIC/LATINO': Ethnicity.NON_HISPANIC_LATINO}
        race = parse_race.get(xstr(wc_metadata['patients_race']).upper(), Race.OTHER)

        contact = Contact(xstr(pdata['phone']),
                          xstr(pdata['email']),
                          sex_dict.get(xstr(wc_metadata['patients_sex']).upper(), Sex.OTHER),
                          ethnicity_dict.get(xstr(wc_metadata['patients_ethnicity']).upper(), Ethnicity.OTHER),
                          race,
                          xstr(wc_metadata['patients_primary_language']))
